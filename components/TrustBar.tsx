...
export default function TrustBar() {
  return (
    <div className="w-full bg-[#111111] border-y border-white/10 py-7 px-4 sm:px-6">
        ...
        <div className="grid grid-cols-3 divide-x divide-white/10">
          {stats.map((stat) => (
            <div key={stat.label} className="flex flex-col items-center gap-1 px-2 sm:px-4">
              ...
              <span className="text-[#888888] text-[10px] sm:text-sm text-center font-semibold leading-tight">{stat.label}</span>
            </div>
          ))}
        </div>
      </div>
