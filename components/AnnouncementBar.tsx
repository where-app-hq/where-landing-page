...
export default function AnnouncementBar() {
  return (
    <a
      href="#signup"
      className="block w-full px-4 py-2.5 transition-opacity hover:opacity-80 announcement-bg"
      style={{ boxShadow: '0 1px 0 0 rgba(252,178,80,0.25)' }}
    >
      <div className="max-w-md mx-auto flex flex-col gap-1.5">
        {/* One line on mobile, with progress bar below on sm+ */}
        <div className="flex items-center justify-center gap-1.5 text-xs font-semibold">
          <span className="text-[#444444] dark:text-[#bbbbbb]">iOS<sup>®</sup> beta is open.</span>
          <span>
            <span className="gradient-text font-extrabold">{WAITLIST_CURRENT.toLocaleString()}</span>
            <span className="text-[#444444] dark:text-[#bbbbbb]"> of {WAITLIST_TOTAL.toLocaleString()} slots filled.</span>
          </span>
        </div>

        {/* Progress bar — hidden on mobile to save space */}
        <div className="hidden sm:block w-full h-1 bg-black/10 dark:bg-white/10 rounded-full overflow-hidden">
          <div
